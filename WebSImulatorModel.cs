...
{   //model of our web application
    ...
    {
        int buffSize = 64; //buffer size constant
        private TcpClient channel;
        NetworkStream strm;
        ASCIIEncoding enc;
        byte[] reader;
        byte[] writer;
        Dictionary<string, string> getPaths;
        //CTOR
        public WebSimulatorModel()
        {
            channel = null;
            strm = null;
        ...
        public bool saveFlightDetails(int perSecond, int seconds, string filename)
        {
            ...
            for (; i < numIterations; ++i) //get data as instructed
            {
                ...
                l.Add(new Tuple<double, double, double, double>(position.Item1, position.Item2, throttle, rudder));
        ...
        public List<Tuple<double, double>> loadFlightDetails(string filename)
        {
            ...
            string line;
            string[] vals;
            List<Tuple<double, double>> toRet = new List<Tuple<double, double>>();
            try
            {
                while ((line = fs.ReadLine()) != null)
                {
                    ...
                    double lon, lat;
                    if (!Double.TryParse(vals[0], out lon) || !Double.TryParse(vals[0], out lat))
                    {
                        return null;
                    }
                    toRet.Add(new Tuple<double, double>(lon, lat));
                }
                fs.Close();
            }
            catch (Exception)
